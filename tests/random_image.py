...
def main():
    ...
    parser.add_argument(
        ...
        dest="monochrome",
        action="store_true",
        help="Make image black-and-white",
    )
    parser.add_argument(
        '--size', '-s',
        dest="requested_size",
        type=int,
        help="Requested size, in kilobytes, of image",
    )
    parser.add_argument(
        '--random', '-r',
        dest="randomize",
        action="store_true",
        help="Make a randomly-colored image",
    )
    parser.add_argument(
        '--output', '-f',
        dest='output_path',
    ...
    if args.requested_size:
        img = None
        if args.randomize:
            img = generate_random_image_by_size(
                args.requested_size,
