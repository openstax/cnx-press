...
from ..utils import produce_hashes_from_filepath
...
def push_legacy_page(model, metadata, submission, db_conn):
    """Push a Page (aka Module) directly into the archive
    ...
    # FIXME The conditions for copying content in have not yet been defined.
    # For now, I'm just blasting it in there.
    ...
    # Check if already in DB: if so, skip
    result = db_conn.execute(
        t.modules.select()
        .where(t.modules.c.uuid == metadata.uuid and
               t.modules.c.major_version == metadata.major_version and
               t.modules.c.minor_version == metadata.minor_version))
    try:
        module_ident = result.fetchone().module_ident
        return (metadata.uuid,
                '{}.{}'.format((metadata.major_version,
                                metadata.minor_version))), module_ident
    except AttributeError:  # NoneType
        pass  # Not present - insert it
    ...
    result = db_conn.execute(t.modules.insert().values(
        ...
        maintainers=metadata.maintainers,
        licensors=metadata.licensors,
        # Carry over parentage information FIXME copy is broked
        parent=None,
        parentauthors=None,
    ...
    # Insert module files (content and resources)
    hashes = produce_hashes_from_filepath(model.file)
    try:
        # Try finding an existing file first
        fileid = db_conn.execute(
            text('SELECT fileid FROM '
                 'files WHERE sha1 = :sha1')
            .bindparams(sha1=hashes['sha1'])
        ).fetchone().fileid
    except AttributeError:
        with model.file.open('rb') as fb:
            result = db_conn.execute(t.files.insert().values(
